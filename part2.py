...
def save_result(sample, user_id, condition, response_source, choices, reading_time_seconds, skipped: bool = False):
    payload = {
        "sample_id": sample["id"],
        "user_id": user_id,
        "condition": condition,
    ...
    fieldnames = [
        ...
        "Q6", "Q7", "reading_time_seconds", "skipped", "timestamp"
    ]
    ...
    if file_exists:
        ...
        if "reading_time_seconds" not in existing_fields:
...
if submit_clicked:
    ...
    save_result(sample, user_id, condition, response_source, choices, reading_time_seconds)
...
if skip_clicked:
    reading_time_seconds = round(time.time() - st.session_state[timer_key], 3)
    save_result(sample, user_id, condition, response_source, choices, reading_time_seconds, skipped=True)
    del st.session_state[timer_key]
    st.info("Skipped.")
    st.rerun()
