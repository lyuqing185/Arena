...
CANDIDATE_FIELD_SPECS = {
    ...
    "candidate_3": ["candidate3", "candidate_3", "Candidate3", "Candidate_3"],
    "candidate_4": ["candidate4", "candidate_4", "Candidate4", "Candidate_4"],
}

ATTENTION_AD_FIELD_SPECS = {
    "candidate_1": ["candidate1_ad_llm", "candidate_1_ad_llm", "Candidate1_ad_llm", "Candidate_1_ad_llm"],
    "candidate_2": ["candidate2_ad_llm", "candidate_2_ad_llm", "Candidate2_ad_llm", "Candidate_2_ad_llm"],
    "candidate_3": ["candidate3_ad_llm", "candidate_3_ad_llm", "Candidate3_ad_llm", "Candidate_3_ad_llm"],
    "candidate_4": ["candidate4_ad_llm", "candidate_4_ad_llm", "Candidate4_ad_llm", "Candidate_4_ad_llm"],
}
...
def load_assignment(path):
    if not os.path.exists(path):
        st.error(f"Assignment file not found. Please check: {path}")
        st.stop()
    ...
    try:
        ...
        st.error(f"Failed to read assignment file: {path}. Error: {e}")
        st.stop()
    ...
    df["display_order"] = df["display_order"].str.upper()

    df["annotation_round_num"] = pd.to_numeric(
        df["annotation_round"], errors="coerce"
    )
    df["task_order_num"] = pd.to_numeric(
        df["task_order"], errors="coerce"
    )

    bad_numeric = df[
        df["annotation_round_num"].isna() | df["task_order_num"].isna()
    ]
    if not bad_numeric.empty:
        st.error(
            "Assignment file has invalid annotation_round or task_order values."
        )
        st.dataframe(
            bad_numeric[
                ["user_id", "task_id", "annotation_round", "task_order"]
            ].head(30),
            use_container_width=True,
        )
        st.stop()

    bad_order = df[~df["display_order"].isin(["AB", "BA"])]
    if not bad_order.empty:
        st.error("Assignment file has invalid display_order values. Only AB and BA are allowed.")
        st.dataframe(
            bad_order[
                ["user_id", "task_id", "sample_id", "field_a", "field_b", "display_order"]
            ].head(30),
            use_container_width=True,
        )
        st.stop()
    ...
    df = df.sort_values(
        ["user_id", "annotation_round_num", "task_order_num", "task_id"],
        kind="stable",
...
def canonical_candidate_field(field: str) -> str:
    raw = str(field or "").strip()
    normalized = raw.lower().replace(" ", "_")

    match = re.search(r"candidate[_]?(\d+)", normalized)
    if match:
        number = int(match.group(1))
        if 1 <= number <= 4:
            return f"candidate_{number}"

    return normalized


def build_task_pool_from_assignment(samples, assignment_df, user_id):
    sample_lookup = {str(sample.get("id", "")).strip(): sample for sample in samples}

    user_rows = assignment_df[assignment_df["user_id"] == user_id].copy()
    user_rows = user_rows.sort_values(
        ["annotation_round_num", "task_order_num", "task_id"],
        kind="stable",
    )
    ...
    duplicated_task_ids = user_rows[user_rows["task_id"].duplicated(keep=False)]
    if not duplicated_task_ids.empty:
        for _, row in duplicated_task_ids.head(30).iterrows():
            errors.append(
                {
                    "error": "duplicate task_id for this user",
                    "task_id": row.get("task_id", ""),
                    "sample_id": row.get("sample_id", ""),
                    "field_a": row.get("field_a", ""),
                    "field_b": row.get("field_b", ""),
                }
            )
    ...
    for _, row in user_rows.iterrows():
        ...
        field_a = canonical_candidate_field(row.get("field_a", ""))
        field_b = canonical_candidate_field(row.get("field_b", ""))
        display_order = str(row.get("display_order", "")).strip().upper()
        ...
        if sample is None:
            errors.append(
                {
                    "error": "sample_id not found in data file",
                    "task_id": task_id,
                    "sample_id": sample_id,
                    "field_a": field_a,
                    "field_b": field_b,
        ...
        if not text_a or not text_b or not ad_a or not ad_b:
            missing_parts = []
            if not text_a:
                missing_parts.append(field_a)
            if not text_b:
                missing_parts.append(field_b)
            if not ad_a:
                missing_parts.append(f"{field_a}_attention_ad")
            if not ad_b:
                missing_parts.append(f"{field_b}_attention_ad")

            errors.append(
                {
                    "error": "missing candidate text or attention ad sentence",
                    "task_id": task_id,
                    "sample_id": sample_id,
                    "field_a": field_a,
                    "field_b": field_b,
        ...
        task_pool.append(
            {
                ...
                "annotation_round": str(row.get("annotation_round", "")).strip(),
                "task_order": str(row.get("task_order", "")).strip(),
                "sample_id": sample_id,
                "batch_id": batch_id,
                "field_a": field_a,
                "field_b": field_b,
...
def normalize_numbered_items(raw: str) -> str:
    ...
    # Only split common list markers such as "1. Item" through "29. Item".
    # Do not let \s consume newlines, otherwise phone numbers such as
    # "741741.\n3. ..." can be misread as numbered-list markers.
    raw = re.sub(
        r"(?<!^)(?<!\n)[ \t]+((?:[1-9]|1\d|2\d)\.[ \t]+)",
        r"\n\1",
        raw,
    ...
    # Preserve intentional blank lines, but avoid very large vertical gaps.
    raw = re.sub(r"\n{3,}", "\n\n", raw)
...
def render_plain_text_block(raw: str):
    text = normalize_numbered_items(str(raw or "")).strip()
    if not text:
        return

    st.html(
        f"<div class='plain-text-block'>{html.escape(text, quote=False)}</div>"
    )
...
def render_candidate_text(text: str):
    ...
    for match in pattern.finditer(str(text)):
        ...
        if normal_part:
            render_plain_text_block(normal_part)

        sponsored_content = re.sub(r"\s+", " ", match.group(1)).strip()
        if sponsored_content:
            st.html(
                "<div class='sponsored-text-block'><strong>Sponsored:</strong> "
                f"{html.escape(sponsored_content, quote=False)}</div>"
            )
...
def strip_outer_context_quote(value: str) -> str:
...
def strip_response_wrapper_tail(value: str) -> str:
    value = str(value or "").strip()
...
def parse_context(context_raw: str):
    ...
    user_response_pattern = re.compile(
        r"\[User:(.*?)\]\s*\[Response:",
        flags=re.DOTALL,
    )
    matches = list(user_response_pattern.finditer(text))

    exchanges = []
    for idx, match in enumerate(matches):
        user_text = strip_outer_context_quote(match.group(1))

        response_start = match.end()
        response_end = matches[idx + 1].start() if idx + 1 < len(matches) else len(text)
        assistant_text = text[response_start:response_end]
        assistant_text = strip_response_wrapper_tail(assistant_text)
        assistant_text = strip_outer_context_quote(assistant_text)
    ...
    with st.expander("Previous conversation context", expanded=True):
        for idx, exchange in enumerate(exchanges):
            if exchange.get("user"):
                st.markdown("**User:**")
                render_plain_text_block(exchange["user"])
...
def prepare_display_task(task):
    display_order = str(task.get("display_order", "")).strip().upper()
    ...
    st.error(
        f"Invalid display_order for task {task.get('task_id', '')}: {display_order}"
    )
    st.stop()
...
def candidate_number_from_field(field: str):
    match = re.search(r"candidate[_\s]*(\d+)", str(field or ""), flags=re.IGNORECASE)
    if not match:
        match = re.search(r"candidate(\d+)", str(field or ""), flags=re.IGNORECASE)
...
def candidate_label_from_field(field: str) -> str:
    number = candidate_number_from_field(field)
    return f"Candidate{number}" if number is not None else ""


def choice_to_candidate_value(choice, display_task):
    if choice == "Version A":
        return candidate_label_from_field(display_task.get("left_field", ""))
    if choice == "Version B":
        return candidate_label_from_field(display_task.get("right_field", ""))
    if choice == "Tie":
        return "Tie"
    return ""
...
def get_ad_position_answer(display_task):
    left_field = display_task.get("left_field", "")
    right_field = display_task.get("right_field", "")

    left_number = candidate_number_from_field(left_field)
    right_number = candidate_number_from_field(right_field)

    if left_number is None and right_number is None:
        return "Tie"
    if left_number is None:
        return candidate_label_from_field(right_field)
    if right_number is None:
        return candidate_label_from_field(left_field)
    if left_number == right_number:
        return "Tie"

    return candidate_label_from_field(left_field if left_number < right_number else right_field)
...
def save_result(
    ...
    payload = {
        ...
        "Q7_choice": "" if skipped else choice_to_candidate_value(choices.get("Q7", ""), display_task),
        "Q7_correct_answer": "" if skipped else get_ad_position_answer(display_task),
        "Q7_is_correct": None
        if skipped
        else (
            choice_to_candidate_value(choices.get("Q7", ""), display_task)
...
def reset_choice_state(task_id):
    ...
    attention_input_key = f"attention_copy_input_{safe_key_part(task_id)}"
    if attention_input_key in st.session_state:
        del st.session_state[attention_input_key]
...
def show_user_id_page():
    ...
    st.markdown(f"Please enter your User ID. It must start with `{USER_ID_PREFIX}`.")
    ...
    if st.button("Continue", use_container_width=True):
        ...
        if not raw_user_id.startswith(USER_ID_PREFIX):
            st.warning(f"Please enter a valid User ID starting with {USER_ID_PREFIX}")
            st.stop()

        user_rows = assignment_df[assignment_df["user_id"] == raw_user_id]
        if user_rows.empty:
            st.warning("This User ID was not found in the assignment file.")
            st.stop()
...
def main():
    ...
    assignment_source_msg = f"Assignment source: Local file ({ASSIGNMENT_PATH})"
    combined_source_msg = f"{data_source_msg}; {assignment_source_msg}"

    if st.session_state.get("_last_data_source_msg") != combined_source_msg:
        print(f"[Ad-Arena] {combined_source_msg}")
        st.session_state["_last_data_source_msg"] = combined_source_msg

    user_id = st.session_state.user_id

    user_assignment_rows = assignment_df[assignment_df["user_id"] == user_id]
    if user_assignment_rows.empty:
        st.error(
            "This User ID is no longer found in the assignment file. "
            "Please return to the login page and check the ID."
        )
        st.stop()

    task_pool, task_errors = build_task_pool_from_assignment(
        data,
        assignment_df,
        user_id,
    )
    ...
    if task_errors:
        st.error(
            "Some assigned tasks cannot be resolved from the data file. "
            "Please fix the assignment or source data before continuing."
        )
        st.dataframe(pd.DataFrame(task_errors).head(50), use_container_width=True)
        st.stop()

    if not task_pool:
        st.error("No assigned tasks were found for this User ID.")
        st.stop()
    ...
    if task is not None:
        ...
        st.markdown(
            f"<div class='meta-text'>Assigned task ({current_position}/{len(task_pool)})</div>",
            unsafe_allow_html=True,
        )

    if task is None:
        st.success("All assigned tasks have been completed.")
        st.stop()
    ...
    render_plain_text_block(display_task.get("question", ""))
    ...
    st.markdown(
        f"<p style='font-size:18px; font-weight:600; margin: 0.5rem 0 0.2rem 0;'>"
        f"Please copy and paste the advertisement sentence from Version {attention_target_version}."
        f"</p>",
        unsafe_allow_html=True,
    )

    attention_text_input = st.text_area(
        "Advertisement sentence input",
        key=f"attention_copy_input_{safe_key_part(display_task['task_id'])}",
        height=90,
