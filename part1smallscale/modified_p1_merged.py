...
Q_LABELS = [
    "Naturalness of flow — Which version reads more smoothly and feels less disruptive?",
    "Ad visibility — In which version is the advertisement more visible?",
    "Helpfulness — Which version better answers the user's question?",
    "Appropriateness — Which version feels more trustworthy and less manipulative?",
    "Overall preference — All things considered, which version is preferred?",
    "Placement — Which version presents the advertisement earlier in the response?",
]

Q_KEYS = ["Q1", "Q2", "Q4", "Q5", "Q6", "Q3"]
...
def normalize_numbered_items(raw: str) -> str:
    """Insert stable line breaks before numbered markers and sponsored blocks.

    It does not insert line breaks after ordinary English periods.
    """
    raw = str(raw or "")

    # Keep markdown headings readable.
    raw = re.sub(r"(?<!\n)\s+(#{1,6})\s+", r"\n\1 ", raw)

    # Put numbered items like 1. 2. 3. on new lines.
    raw = re.sub(r"(?<!^)(?<!\n)\s+(\d+\.\s+)", r"\n\1", raw)

    # Put bracketed sponsored blocks on their own line.
    raw = re.sub(
    ...
    # Also handle plain text Sponsored: if it appears without brackets.
    raw = re.sub(
        r"(?<!\n)\s+(Sponsored:\s*)",
        r"\n\1",
        raw,
...
def escape_markdown_math_dollars(raw: str) -> str:
    """Prevent Streamlit Markdown from treating dollar amounts such as $10 as LaTeX."""
...
def is_sentence_boundary(text: str, idx: int):
    ...
    # Allow closing quotes/brackets after sentence punctuation.
    j = idx + 1
    while j < len(text) and text[j] in {'"', "'", "”", "’", ")", "]", "}"}:
        j += 1

    # After optional closing punctuation, the next character should be whitespace or end of line.
    if j < len(text) and not text[j].isspace():
        return False

    # Do not split numbered list markers like "10. Dark Matter".
    # But do allow sentence endings like The 100." Stan...
    if ch == "." and prev_char.isdigit():
        left_start = text[max(0, idx - 6):idx + 1]
        right_after_closing = text[j:j + 20]

        # If this looks like a list marker at the beginning of a line, do not split.
        line_start = text.rfind("\n", 0, idx) + 1
        prefix_on_line = text[line_start:idx + 1]
        if re.match(r"^\s*\d+\.$", prefix_on_line):
            return False

        # Otherwise, allow numeric sentence endings such as "The 100."
        return True
...
def _split_plain_attention_text(text: str, start_idx: int = 0):
    ...
    for line_no, line in enumerate(lines):
        ...
        for idx, ch in enumerate(line):
            # Skip characters that have already been consumed as closing quotes/brackets
            # after a sentence-ending punctuation mark.
            if idx < last_consumed:
                continue
            ...
            if sentence_start is not None and is_sentence_boundary(line, idx):
                ...
                # Include closing quotes/brackets after sentence punctuation.
                while end_idx < len(line) and line[end_idx] in {'"', "'", "”", "’", ")", "]", "}"}:
                    end_idx += 1

                parts.append(
                    {
                        "text": line[sentence_start:end_idx],
                        "clickable": True,
                        "idx": sentence_idx,
                        "sponsored": False,
                    }
...
def render_clickable_attention_text(text: str, side: str, task_id: str):
    ...
    for part in split_into_sentence_parts(text):
        ...
        if part.get("clickable"):
            ...
            if "\n" in part_text:
                # Use a compact line break instead of a full markdown paragraph.
                st.html("<br>")
            elif part_text.strip():
                st.html(f"<span>{html.escape(part_text)}</span>")
...
def get_question_display_order(task_id: str):
    """Randomly place Q3 while keeping all other questions in their original order.

    The displayed order may change, but the saved field names remain Q1_choice,
    Q2_choice, ..., Q6_choice. Therefore Q3_choice always means Placement.
    """
    key = f"question_display_order_{safe_button_key_part(task_id)}"

    current_order = st.session_state.get(key)
    if isinstance(current_order, list) and set(current_order) == set(Q_KEYS):
        return current_order

    fixed_order_without_q3 = [q_key for q_key in Q_KEYS if q_key != "Q3"]
    insert_position = secrets.randbelow(len(fixed_order_without_q3) + 1)
    display_order = fixed_order_without_q3.copy()
    display_order.insert(insert_position, "Q3")
