...
# Put part1_sponsored_rewritten.xlsx in the same folder as this .py file.
DATA_PATH = os.getenv(
    "DATA_PATH",
    os.path.join(BASE_DIR, "part1_sponsored_rewritten.xlsx")
)

SUPABASE_TABLE = "part1_results"
SURVEY_PREFIX = "sponsored"

# Each row becomes one comparison task: candidate2' vs candidate4'.
...
def is_sentence_boundary(text: str, idx: int):
    ...
    if ch == "." and prev_char.isdigit():
        # But allow quoted sentence endings like The 100."
...
def render_attention_check(display_task):
    ...
    left_ad = extract_sponsored_text(display_task.get("left_prime", ""))
    right_ad = extract_sponsored_text(display_task.get("right_prime", ""))

    if not left_ad:
        left_ad = display_task.get("matched_ad", "")
    if not right_ad:
        right_ad = display_task.get("matched_ad", "")
...
def prepare_display_task(task):
    ...
    if display_order == "AB":
        return {
            **task,
            ...
            "left_prime": task.get("prime_a", ""),
            "right_field": task["field_b"],
            "right_text": task["text_b"],
            "right_prime": task.get("prime_b", ""),
    ...
    return {
        **task,
        ...
        "left_prime": task.get("prime_b", ""),
        "right_field": task["field_a"],
        "right_text": task["text_a"],
        "right_prime": task.get("prime_a", ""),
        "display_order": display_order,
    }
...
def get_question_display_order(task_id: str):
    ...
    Q2_choice, ..., Q6_choice. Therefore Q3_choice always means Ad noticeability.
    """
...
def main():
    ...
    if not task_pool:
        st.error(
            "No valid candidate pairs were found. Please check whether candidate2' "
            "and candidate4' fields are present and non-empty."
        )
        st.stop()
